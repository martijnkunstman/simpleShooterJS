...
export default class Controls {
    ...
    lastDraw;
    fps;
    constructor(damping = 0.05, friction = 0.05, force = 0.15) {
    ...
    update(input) {
        ...
        if (input.keyUpActive) {
            this.vector.plus(new Vector(0, -this.force));
            keyActive = true;
        }
        ...
        if (input.keyLeftActive) {
            this.vector.plus(new Vector(-this.force, 0));
            keyActive = true;
        }
    ...
    draw(input) {
        let canvas = document.getElementById("game2");
